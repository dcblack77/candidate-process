...
describe("BulkCvUploadPanel", () => {
    ...
    it("muestra la vista previa con nombre opcional y bloquea formatos no admitidos", async () => {
        ...
        await pickFiles([file("cv_ana-perez.pdf"), file("notas.md", 5, "text/markdown")]);
        ...
        await userEvent.click(screen.getByRole("button", { name: "Quitar notas.md" }));
    ...
    it("un archivo de más de 10 MB queda marcado y bloquea el lote", async () => {
        ...
        const big = new File([new Uint8Array(10 * 1024 * 1024 + 1)], "gigante.pdf", {
            type: "application/pdf",
        });
        await pickFiles([big]);
        expect(screen.getByText("Supera los 10 MB.")).toBeInTheDocument();
        expect(screen.getByRole("button", { name: "Importar 1 CV" })).toBeDisabled();
    });
    ...
    it("envía multipart con `files` (+ `names` solo si se escribió alguno), pinta el job y hace polling hasta terminar", async () => {
        ...
        await userEvent.click(screen.getByRole("button", { name: "Importar 2 CVs" }));
        ...
        const post = calls.find((call) => call.url === "/api/candidates/cv/bulk")!;
        expect(post.init.method).toBe("POST");
        const form = post.init.body as FormData;
        expect(form.getAll("files")).toHaveLength(2);
        expect((form.getAll("files")[0] as File).name).toBe("cv_ana-perez.pdf");
        expect(JSON.parse(String(form.get("names")))).toEqual([null, "Luis Gómez"]);
        ...
        expect(within(jobView).getByRole("link", { name: "Ana Perez" })).toHaveAttribute(
            "href",
            "/candidates/c1",
        );
        expect(
    ...
    it("sin nombres escritos no viaja el campo names", async () => {
        ...
        await userEvent.click(screen.getByRole("button", { name: "Importar 1 CV" }));
        await waitFor(() => {
            const post = calls.find((call) => call.url === "/api/candidates/cv/bulk");
    ...
    it("cancelar llama a DELETE y muestra la cancelación", async () => {
        ...
        await userEvent.click(screen.getByRole("button", { name: "Importar 2 CVs" }));
        const cancel = await screen.findByRole("button", { name: "Cancelar lote" });
    ...
    it("los límites del lote muestran el mensaje concreto del backend", async () => {
        ...
        expect(screen.getByRole("button", { name: "Importar 2 CVs" })).toBeEnabled();
    });
    ...
    it("bulkCvApi.start no adjunta names si todos son null", async () => {
        const { calls } = installFetchMock({
            "POST /api/candidates/cv/bulk": () => jsonResponse(job(), 202),
        });
        await bulkCvApi.start([file("a.pdf")], [null]);
